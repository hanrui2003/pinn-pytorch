...
\begin{equation*}
	\begin{aligned}
		Lu=f, u = \sum_{m=1}^M u_m \phi_m(\boldsymbol{x}) \\
		Lu = \sum_{m=1}^M u_m L\phi_m(\boldsymbol{x}) = f \\
		\sum_{m=1}^M u_m L\phi_m(\boldsymbol{x_j}) = f_j \\
		\boldsymbol{Au} = \boldsymbol{f}
	\end{aligned}
\end{equation*}
问题就转换为了最小二乘问题，其中u就是最后待求解的外层权重参数，这个算法有潜在的问题，就是如果没有源项，即f=0，则求出来的u=0。
...
\textcolor{red}{注：这里把定义域划分为$M_p$个小区间，$\{\boldsymbol{x}_n\}$是每个小区间的中点}
\begin{equation}
...
\end{equation*}
\textcolor{red}{注：右边的$\Sigma$对应的是一个全连接网络，$J_n$表示这个网络的隐层维度（神经元个数），左边的$\Sigma$表示用单位分解技术将它们组合。$M_p$表示划分的区间个数，对应着神经网络的个数。}\\


\subsection{参考文献}
\begin{enumerate}
	\item 介绍RFM的：J. Chen, X. Chi, W. E, and Z. Yang, Bridging traditional and machine learning based algorithms for solving pdes: The random feature method, Journal of Machine Learning, 1 (2022), pp. 268–298.
	\item 关于神经网络MSE缺陷的文章： C. Wang, S. Li, D. He, and L. Wang, Is $ L^ 2$ Physics Informed Loss Always Suitable for Training Physics Informed Neural Network?. Advances in Neural Information Processing Systems, 35 (2022), pp. 8278-8290.
\end{enumerate}
