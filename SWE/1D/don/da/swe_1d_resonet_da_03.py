...
class SWEICDataset(Dataset):
    ...
    def __init__(self, branch_train):
        ...
        h_label = branch_train[:, :101].reshape(-1, 1)
...
class SWEObsDataset(Dataset):
    ...
    def __init__(self, branch_train):
        ...
        h_label = branch_train[:, 101:141].reshape(-1, 1)
        v_label = branch_train[:, 141:].reshape(-1, 1)
        label = np.hstack((h_label, v_label))
        print('obs dataset size (mb):', (sys.getsizeof(z_train_nn) + sys.getsizeof(label)) / 1024 / 1024)
...
if "__main__" == __name__:
    ...
    for idx, seq in enumerate(train_data):
        ...
        obs = np.concatenate((ic[:101], obs_h_1, obs_h_2, obs_h_3, obs_h_4, obs_v_1, obs_v_2, obs_v_3, obs_v_4))
        branch_data = np.vstack((branch_data, obs))

    # 构建网络网络结构
    model = ResidualNetwork(input_size=183, hidden_size=128, num_blocks=3, output_size=2)
    model.apply(weights_init)
    model.to(device)
    print("model:\n", model)

    old_model_weights_path = 'swe_1d_resonet_da_03_1e-3.pt'
    new_model_weights_path = 'swe_1d_resonet_da_03_5e-5.pt'
    save_threshold = 5e-5
