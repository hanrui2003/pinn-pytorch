import numpy as np
import torch
from swe_1d_don_01 import SWENet
...
if "__main__" == __name__:
    ...
    model = torch.load('swe_1d_don_01_0.5_e-5.pt', map_location=torch.device('cpu'))
    print("model", model)
    ...
    m0 = np.zeros(N_x)
    u0 = np.hstack((h0, m0))
    u0_test = torch.from_numpy(u0).float()
    ...
    u2 = solve2(u0, np.linspace(0, 0.05, N_t))
    h2 = u2[:, :N_x]
    ...
    anim.save("{}.mp4".format("swe_1d_don_01_2"), writer=mpeg_writer)
