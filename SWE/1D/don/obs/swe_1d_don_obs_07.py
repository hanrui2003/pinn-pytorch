...
class SWEObsDataset(Dataset):
    ...
    def __init__(self, o_train):
        ...
        t_train_obs = np.array([.1, .2, .3, .4, .5])[:, None]
...
class SWEBCDataset(Dataset):
    ...
    def __init__(self, o_train):
        t_train_bc = np.linspace(0, .5, 51)[:, None]
...
if "__main__" == __name__:
    ...
    train_data = np.load('swe_1d_rbf_sample_1.npy')[:10]

    # 观测位置选择x=0.5，对应的高度和速度的索引为
    # 遍历每个样本序列
    delta_index = np.array([10, 20, 30, 40, 50])
    ic_index = [i for i in range(101)]
    obs_data = np.empty((0, 212))
    for seq in train_data:
        random_ics = seq[ic_index, :]
        # 对于每个初值，不妨选取中间的点作为观测点，每隔0.1取一个观测值，对应的索引间隔就是10
        for idx, ic in enumerate(random_ics):
            obs_index = ic_index[idx] + delta_index
    ...
    # 构建网络网络结构
    branch_layers = [212, 200, 100, 50, 50, 50]
    ...
    torch.save(model, 'swe_1d_don_obs_07.pt')
