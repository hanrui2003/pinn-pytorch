...
class SWEObsDataset(Dataset):
    ...
    def __init__(self, o_train):
        ...
        h_label = o_train[:, :106].reshape(-1, 1)
        v_label = np.hstack((np.zeros((o_train_count, 101)), o_train[:, -5:])).reshape(-1, 1)
...
if "__main__" == __name__:
    ...
    torch.save(model, 'swe_1d_don_tbc_obs_02.pt')
