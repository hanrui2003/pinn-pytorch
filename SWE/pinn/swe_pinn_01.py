...
class SWENet(nn.Module):
    ...
    def loss_pde(self, z_pde):
        ...
        dq = autograd.grad(q, z_pde, torch.ones_like(q), create_graph=True)[0]
        ds = autograd.grad(s, z_pde, torch.ones_like(s), create_graph=True)[0]
        q_x = dq[:, [0]]
        s_y = ds[:, [1]]

        lhs_ = torch.hstack((u_t, v_t, eta_t))
        rhs_ = torch.hstack((-self.g * eta_x, -self.g * eta_y, -q_x - s_y))
...
if "__main__" == __name__:
    ...
    torch.save(model, 'swe_pinn_01_gzz_100000_e-4.pt')
