...
class FCN(nn.Module):
    ...
    def loss_pde(self, x_pde):
        ...
        loss_pde_x = self.loss_func(x_t, 4 * y_nn)
        loss_pde_y = self.loss_func(y_t, -4 * x_nn)
        loss_pde_z = self.loss_func(z_t, 16 * x_nn * y_nn)
        return loss_pde_x + loss_pde_y + loss_pde_z
...
for i in range(epochs):
    ...
    if (i + 1) % 1000 == 0:
        print('epoch :', i + 1, 'lr :', optimizer.param_groups[0]['lr'])
