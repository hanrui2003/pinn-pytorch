...
class FCN(nn.Module):
    ...
    def loss_pde(self, x_pde):
        ...
        y_t = autograd.grad(y_hat, x_pde, torch.ones_like(y_hat), create_graph=True)[0]
        loss_pde = self.loss_func(y_t, torch.sin(y_hat))
...
if "__main__" == __name__:
    ...
    for j in range(1, total_points):
        k1 = np.sin(u[j - 1])
        k2 = np.sin(u[j - 1] + h / 2 * k1)
        k3 = np.sin(u[j - 1] + h / 2 * k2)
        k4 = np.sin(u[j - 1] + h * k3)
    ...
    while True:
        ...
        if lr < 1e-7 or loss.item() < 0.01:
            print('epoch :', epoch, 'lr :', lr, 'loss :', loss.item())
            break

    torch.save(PINN, 'simple_ode_11.pt')
    PINN = PINN.cpu()
    nn_predict = PINN(x_test[:, None]).detach().numpy()

    fig, ax = plt.subplots()
    ax.plot(x_test, nn_predict, 'b-', linewidth=2, label='predict')
    ax.plot(x_test, u, 'r--', linewidth=2, label='truth')
    ax.set_title('simple ode')
    ax.set_xlabel('t', color='black')
    ax.set_ylabel('f(t)', color='black', rotation=0)
    ax.legend(loc='upper right')
    plt.savefig('./images/simple_ode_11.png')
    # plt.close()
    plt.show()
