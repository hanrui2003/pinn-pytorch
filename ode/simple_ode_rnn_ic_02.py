...
class FCN(nn.Module):
    def __init__(self, x_lb, x_ub, y_train_ic, in_features=1, hidden_size=16, num_layers=1, out_features=1):
        super().__init__()
        self.x_lb = x_lb
        self.x_ub = x_ub
        self.y_train_ic = y_train_ic
        self.h_0 = torch.tensor([[np.sin(-0.005)]]).float()
        print("h_0 : ", self.h_0)
        self.loss_func = nn.MSELoss(reduction='mean')
        self.rnn1 = nn.RNN(input_size=in_features, hidden_size=hidden_size, num_layers=num_layers, batch_first=True)
        self.rnn2 = nn.RNN(input_size=hidden_size, hidden_size=out_features, num_layers=num_layers, batch_first=True)
...
if "__main__" == __name__:
    ...
    torch.save(PINN, 'simple_ode_rnn_ic_02.pt')
    PINN.cpu()
    ...
    plt.savefig('./figure/simple_ode_rnn_ic_02.png')
    # plt.show()
