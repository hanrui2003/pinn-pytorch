...
class FCN(nn.Module):
    ...
    def loss_pde(self, x_pde):
        ...
        loss_x = self.loss_func(x_t, x_nn + y_nn / 2)
        loss_y = self.loss_func(y_t, y_nn)
...
if "__main__" == __name__:
    ...
    torch.save(PINN, 'simple_ode_12.pt')
    PINN = PINN.cpu()
    ...
    ax.plot(x_test, u[:, [0]], 'r--', linewidth=2, label='x-truth')
    ax.plot(x_test, u[:, [1]], 'c--', linewidth=2, label='y-truth')
    ax.set_title('simple ode')
    ax.set_xlabel('t', color='black')
    ax.set_ylabel('f(t)', color='black', rotation=0)
    ax.legend(loc='upper right')
    plt.savefig('./images/simple_ode_12.png')
    plt.close()
    # plt.show()
