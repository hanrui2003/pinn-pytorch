...
class FCN(nn.Module):
    def __init__(self, layers) -> None:
        ...
        self.x_dnn = nn.ModuleList([nn.Linear(layers[k], layers[k + 1]) for k in range(len(layers) - 1)])
        self.y_dnn = nn.ModuleList([nn.Linear(layers[k], layers[k + 1]) for k in range(len(layers) - 1)])
    ...
    def loss_pde(self, x_pde):
        ...
        loss1 = self.loss_func(x_t, y_nn)
        loss2 = self.loss_func(y_t, -x_nn)
        return loss1 + loss2
