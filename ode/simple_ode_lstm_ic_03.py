...
class FCN(nn.Module):
    def __init__(self, x_lb, x_ub, y_train_ic, input_size=1, hidden_size=16, num_layers=2, out_features=1):
        super().__init__()
        self.x_lb = x_lb
        self.x_ub = x_ub
        self.y_train_ic = y_train_ic
        self.h0 = torch.nn.init.uniform_(torch.empty(num_layers, hidden_size), -1, 1)
        self.c0 = torch.nn.init.uniform_(torch.empty(num_layers, hidden_size), -1, 1)
...
if "__main__" == __name__:
    ...
    torch.save(PINN, 'simple_ode_lstm_ic_03.pt')
    PINN.cpu()
    ...
    plt.savefig('./figure/simple_ode_lstm_ic_03.png')
    # plt.show()
