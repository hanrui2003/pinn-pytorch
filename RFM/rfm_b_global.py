...
def pre_define(M_p, M, J_n):
    ...
    """
    # 全局
    global_model = RFM_rep_b_global(d=1, M=M)
...
def assemble_matrix(global_model, local_models, points, M_p, M, J_n, Q, lamb):
    """
    L表示局部local，G代表全局global
    models：模型，其数量等于M_p
    ...
    # 边界点
    boundary_G = torch.tensor([[X_min], [X_max]], dtype=torch.float64)
    point_G = torch.tensor(points, requires_grad=True)

    A_I_L = np.zeros([M_p * Q + 1, M_p * J_n])  # PDE term
    # 左右两个边界点，所以横轴维度为2
    # 每个边界点经过一个神经网络的输出是J_n维，一共M_p个网络，所以纵轴维度是M_p * J_n
    A_B_L = np.zeros([2, M_p * J_n])  # boundary condition

    f = np.zeros([M_p * Q + 3, 1])

    # 边界点
    boundary_L = torch.tensor([[X_min], [X_max]], dtype=torch.float64)
    point_L = torch.tensor(points, requires_grad=True)

    # 全局计算
    pde_out_G = global_model(point_G)
    pde_values_G = pde_out_G.detach().numpy()

    boundary_out_G = global_model(boundary_G)
    boundary_values_G = boundary_out_G.detach().numpy()
    # 记录一阶导和二阶导
    grad2_G = []
    # 当前区间的配点求导，由于torch的局限，不能多维函数一起求导，所以只能循环
    for i in range(M):
        g1_G = torch.autograd.grad(outputs=pde_out_G[:, i], inputs=point_G,
                                   grad_outputs=torch.ones_like(pde_out_G[:, i]),
                                   create_graph=True, retain_graph=True)[0]

        g2_G = torch.autograd.grad(outputs=g1_G[:, 0], inputs=point_G,
                                   grad_outputs=torch.ones_like(pde_out_G[:, i]),
                                   create_graph=False, retain_graph=True)[0]

        grad2_G.append(g2_G.squeeze().detach().numpy())
    grad2_G = np.array(grad2_G).T
    A_I_G = grad2_G - lamb * pde_values_G

    # boundary conditions
    A_B_G = boundary_values_G

    A_G = np.concatenate((A_I_G, A_B_G), axis=0)

    # 局部计算：遍历每个区间，区间数M_p =========== start
    for n in range(M_p):
        # forward and grad
        # 当前单位分解区间的配点进入对应的神经网络
        pde_out_L = local_models[n](point_L)
        pde_values_L = pde_out_L.detach().numpy()

        boundary_out_L = local_models[n](boundary_L)
        boundary_values_L = boundary_out_L.detach().numpy()
        # 记录一阶导和二阶导
        grad2_L = []
        # 当前区间的配点求导，由于torch的局限，不能多维函数（J_n维）一起求导，所以只能循环
        for i in range(J_n):
            g1_L = torch.autograd.grad(outputs=pde_out_L[:, i], inputs=point_L,
                                       grad_outputs=torch.ones_like(pde_out_L[:, i]),
                                       create_graph=True, retain_graph=True)[0]

            g2_L = torch.autograd.grad(outputs=g1_L[:, 0], inputs=point_L,
                                       grad_outputs=torch.ones_like(pde_out_L[:, i]),
                                       create_graph=False, retain_graph=True)[0]

            grad2_L.append(g2_L.squeeze().detach().numpy())
        grad2_L = np.array(grad2_L).T
        ...
        Lu_L = grad2_L - lamb * pde_values_L

        # Lu = f condition
        A_I_L[:, n * J_n:(n + 1) * J_n] = Lu_L

        # boundary conditions
        A_B_L[:, n * J_n:(n + 1) * J_n] = boundary_values_L

    # 遍历每个区间 =========== end

    A_L = np.concatenate((A_I_L, A_B_L), axis=0)
...
def main(M_p, M, J_n, Q, lamb):
    ...
    # matrix define (Au=f)
    A, f = assemble_matrix(global_model, local_models, points, M_p, M, J_n, Q, lamb)
    print('***********************')
    ...
    # test
    error = test(global_model, local_models, M_p, M, J_n, Q, w)
...
def test(global_model, local_models, M_p, M, J_n, Q, w):
    ...
    A_L = np.zeros((len(point), M_p * J_n))
    for n in range(M_p):
        out = local_models[n](point)
        values = out.detach().numpy()
        A_L[:, n * J_n: (n + 1) * J_n] = values
...
if __name__ == '__main__':
    ...
    # M_p = 2
    # main(M_p, M, J_n, Q, lamb)
    RFM_Error = np.zeros([100, 3])
    count = 0
    for i in range(100):  # the number of PoU regions
        # 划分的区间数
        M_p = 32
        RFM_Error[i, 0] = int(M_p * J_n)
        RFM_Error[i, 1], RFM_Error[i, 2] = main(M_p, M, J_n, Q, lamb)
        if RFM_Error[i, 1] < 1e-10:
            count += 1
