...
我们取$\lambda = 4$，真解$u(x) = \sin(3\pi x + \frac{3\pi}{20}) \cos(2\pi x + \frac{\pi}{10}) + 2$，则$c_1, c_2$和$f(x)$可相应的计算出来。对方程使用RFM进行数值求解，每个子区域的特征函数和配点的数量分别为$J_n=50$和$Q_n=50$，子区域划分的个数依次取$M_p =2, 4, 8, 16, 32 $，单位分解函数的策略分别为：$\psi^a$、$\psi^b$和无$\psi$，结果见表格\ref{tab:Helmholtz}。{\color{red} 见图}
...
由图和表格\ref{tab:Helmholtz}，可以看出在相同超参数使用RFM求解Helmholtz方程的情况下，无$\psi$策略相比于使用$\psi^a$和$\psi^b$作为单位分解函数，$L^\infty$误差和$L^2$误差降低了好几个阶。数值结果验证了改进后的RFM方法对于静态方程的有效性。
...
我们取$\nu=0.01$，真解$u(x, t) = [2\cos(\pi x + \frac{\pi}{5}) + \frac{3}{2}\cos(2\pi x - \frac{3\pi}{5})] [2\cos(\pi t + \frac{\pi}{5}) + \frac{3}{2}\cos(2\pi t - \frac{3\pi}{5})]$，则$f(x,t), g_x(t), g_2(t), h(x)$可相应的计算出来。对方程使用ST-RFM进行数值求解，每个子区域的特征函数的数量为$J_n=300$，每个子区域的空间维度的配点数量为$N_x = 30$，每个子区域的时间维度的配点数量为$N_t = 30$，所以每个子区域的配点数量为$Q_n = N_x \times N_t = 900$，空间维度的划分数量为$N_x = 5$，时间维度的划分数量依次取$N_t = 1, 2, 3, 4, 5$，单位分解函数的策略分别为：$\psi^a$、$\psi^b$和无$\psi$，结果见表格\ref{tab:Helmholtz}。{\color{red} 见图}
...
\begin{table}
	...
	\begin{tabular}{|c|c|c|c|c|c|c|}
		\hline
		\multirow{2}{*}{$N_t$} & \multicolumn{2}{c|}{$\psi^a$} & \multicolumn{2}{c|}{$\psi^b$} & \multicolumn{2}{c|}{无$\psi$} \\
		\cline{2-7}
		& $L^\infty$ error & $L^2$ error & $L^\infty$ error & $L^2$ error & $L^\infty$ error & $L^2$ error\\
		\hline
		1 &  &  & 1.11E+01 & 1.96E+00 & 1.64E-09 & 2.69E-10\\
		\hline
		2 &  &  & 1.50E-07 & 1.51E-08 & 1.75E-10 & 4.07E-11\\
		\hline
		3 &  &  &  &  & 7.19E-11 & 8.48E-12\\
		\hline
		4 & 1.08E-08 & 1.65E-09 &  &  & 7.45E-11 & 1.11E-11\\
		\hline
		5 & 1.26E-08 & 1.79E-09 & 7.86E-07 & 4.85E-08 & 5.32E-11 & 5.70E-12\\
		\hline
	\end{tabular}
\end{table}
