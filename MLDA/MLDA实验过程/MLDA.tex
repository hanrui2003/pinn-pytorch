...
\subsection{随机特征函数}
\cite{rfm} 随机特征函数（random feature function, RFF）就是特征向量随机生成的函数。对于机器学习方法来说，特征向量就是网络的权重和偏置，特征函数的随机生成就是网络权重和偏置的随机初始化步骤。因此从机器学习的框架下看，RFM就是利用 $M$ 个定义在 $\Omega$ 上的网络基函数 $\{\phi_m\}$ 的线性组合来表示数值解
...
用于解决PDEs的随机特征方法（RFM），这是传统算法和基于机器学习的算法之间的自然桥梁。RFM基于以下几个众所周知的思想的组合：1. 使用随机特征函数表示近似解的方法；2. 用配置方法处理PDE；3. 用罚函数法处理边界条件，这使我们可以在相同的基础上处理边界条件和PDE。

类似于神经网络的随机特征模型，选取随机特征函数作为基函数，构造一个内部参数固定的两层神经网络。在此基础上拼接一个线性输出层，
