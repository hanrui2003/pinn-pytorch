...
深度算子网络（Deep Operator Network, DeepONet）是基于算子通用逼近定理衍生出来的一种神经网络架构，旨在为为求解参数化的偏微分方程提供更通用的方案。
...
\begin{itemize}
	...
	这是一个三层神经网络，网络的输入是偏微分的参数$u(x)$，因为$u(x)$是一个函数，所以要取其离散格式$u(x_j),j=1, 2, \cdots, m$作为输入。我们称这个网络为分支网络。
\end{itemize}
