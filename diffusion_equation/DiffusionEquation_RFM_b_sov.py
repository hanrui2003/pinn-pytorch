...
class RFM_rep_b(nn.Module):
    def __init__(self, J_n, x_max, x_min, t_min, t_max, n_x, n_t, M_p):
        """
        ...
        self.phi_x = nn.Sequential(nn.Linear(1, self.J_n, bias=True), nn.Tanh())
        self.phi_t = nn.Sequential(nn.Linear(1, self.J_n, bias=True), nn.Tanh())
...
def pre_define(M_p, J_n):
    ...
    for i in range(M_p[0]):
        ...
        for j in range(M_p[1]):
            ...
            # 每个单位分解区间对应一个FCN
            model = RFM_rep_b(J_n=J_n, x_min=x_min, x_max=x_max, t_min=t_min, t_max=t_max, n_x=i, n_t=j, M_p=M_p)
...
def test(models, M_p, J_n, Q, w):
    ...
    plot(X, T, true_values.reshape((X.shape[0], X.shape[1])), X, T, numerical_values.reshape((X.shape[0], X.shape[1])))
