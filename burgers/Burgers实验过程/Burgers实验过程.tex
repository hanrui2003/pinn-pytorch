...
\begin{equation}
	\begin{aligned}
		u^{n+1}_i &= u^n_i - \frac{\Delta t}{2\Delta x} (u^n_{i+1} - u^n_{i-1}) u^n_i+ \nu \frac{\Delta t}{(\Delta x)^2} (u^n_{i-1} - 2u^n_i + u^n_{i+1}) \\
		\mathbf{u}^{n+1} &= \mathbf{u}^{n} - \frac{\Delta t}{2\Delta x} (D^1 \cdot \mathbf{u}^n) * \mathbf{u}^n) +  \nu \frac{\Delta t}{(\Delta x)^2} \cdot D^2 \cdot \mathbf{u}^n
	\end{aligned}
\end{equation}
...
\end{bmatrix}
\end{equation}
{\color{red} 这里需要注意的有：$\nu = \frac{0.01}{\pi}$；每次迭代完 $\mathbf{u}^{n+1}$ 的第0个元素和第N个元素是不对的，此时用边值条件重置下；* 表示两个向量对应元素相乘。}
