...
def f_real(x, t):
    ...
    u0x = -2 * Pi * np.sin(Pi * x + Pi / 5) - 3 * Pi * np.sin(2 * Pi * x - 3 * Pi / 5)
    ux = u0x * u1

    u0xx = -2 * Pi * Pi * np.cos(Pi * x + Pi / 5) - 6 * Pi * Pi * np.cos(2 * Pi * x - 3 * Pi / 5)
    uxx = u0xx * u1

    f = ut + v * ux - nu * uxx + D * u
...
def main(Nx, Nt, M, Qx, Qt):
    ...
    torch.save(models, 'convection_diffusion_no_psi.pt')
    np.savez('convection_diffusion_no_psi.npz', w=w,
