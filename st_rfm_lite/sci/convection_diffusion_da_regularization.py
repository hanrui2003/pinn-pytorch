...
def cal_matrix(models, Nx, Nt, M, Qx, Qt, pde_points, ic_points, bc_points, obs_points):
    ...
    # 去掉背景场，即初边值
    # A = np.concatenate((A_P, A_O), axis=0)
    # f = np.concatenate((f_P, f_O), axis=0)
...
def main(Nx, Nt, M, Qx, Qt):
    ...
    t_obs = np.linspace(T_min, T_max, 3)[1:]
    x_obs = np.linspace(X_min, X_max, 6)[1:-1]
    ...
    torch.save(models, 'convection_diffusion_da_with_bg.pt')
    np.savez('convection_diffusion_da_with_bg.npz', w=w,
