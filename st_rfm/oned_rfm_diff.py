...
def cal_matrix(models,points,Nx,Nt,M,Qx,Qt,initial=None,tshift=0):
    ...
    for k in range(Nx):
        for n in range(Nt):
            ...
            f_1[k*Nt*Qx*Qt + n*Qx*Qt : k*Nt*Qx*Qt + n*Qx*Qt + Qx*Qt, :] = vanal_f(f_in[:,0],f_in[:,1] + tshift).reshape(-1,1)
            ...
            A_1[k*Nt*Qx*Qt + n*Qx*Qt : k*Nt*Qx*Qt + n*Qx*Qt + Qx*Qt, M_begin : M_begin + M] = grads_t - eqn.nu * grads_2_xx# - np.power(values[:Qx, :Qt, :], 2.0).reshape(-1, M)
            ...
            # C0 continuity on x
            if k > 0:
                C_0x[(k-1)*Qt*Nt + n*Qt: (k-1)*Qt*Nt + (n+1)*Qt, M_begin: M_begin + M] = values[0, :Qt, :]
            if k < Nx - 1:
                C_0x[k*Nt*Qt + n*Qt: k*Nt*Qt + (n+1)*Qt, M_begin: M_begin + M] = -values[-1, :Qt, :]

            # C1 continuity on x
            if k > 0:
                C_1x[(k-1)*Qt*Nt + n*Qt: (k-1)*Qt*Nt + (n+1)*Qt, M_begin: M_begin + M] = grads[0, 0, :Qt, :]
            if k < Nx - 1:
...
def main(Nx,Nt,M,Qx,Qt,tf=1.0,time_block=1,plot = False,moore = False, img_save_dir=None):
    ...
    if plot:
        ...
        utils.visualize(true_values, numerical_values, x0=eqn.a1, x1=eqn.b1, t0=0, t1=tf, savedir=img_save_dir, eqname=eqn.name, mename="rfm")
...
if __name__ == '__main__':
    ...
    utils.record(main, eqn.name, recursive_times=n, tf=tf, time_blocks=time_blocks, Nxs=Nxs, Nts=Nts, Ms=Ms, Qxs=Qxs, Qts=Qts, eqn=eqn.name, method="rfm")
