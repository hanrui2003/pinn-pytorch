...
class visualize_dyn_space_1d(object):
    ...
    def update(self, i):
        ...
        self.ax.set_title(
            f"t={i * (self.t1 - self.t0) / self.t_bins:.2f}, range=({ydata.max():.2f}, {ydata.min():.2f})")
...
def visualize(true_values, numerical_values, x0, x1, t0, t1, savedir=None, eqname="diff", mename="rfm",
              img_format="eps"):
    ...
    visualize_single(true_values, x0, x1, t0, t1,
                     savename=os.path.join(savedir, f"{eqname}-{mename}-trusol.{img_format}"))
    visualize_single(numerical_values, x0, x1, t0, t1,
                     savename=os.path.join(savedir, f"{eqname}-{mename}-numsol.{img_format}"))
    visualize_single(epsilon, x0, x1, t0, t1, savename=os.path.join(savedir, f"{eqname}-{mename}-eps.{img_format}"))


def visualize_2d_single(values: np.ndarray, x0: float, x1: float, y0: float, y1: float, t0: float, t1: float,
...
def visualize_2d_wo(eqn, values: np.ndarray, x0: float, x1: float, y0: float, y1: float, savename="visualization.png"):
    ...
    im = ax.plot_trisurf(x.flatten(), y.flatten(), values.flatten(), triangles=tri.simplices[mask],
...
def visualize_3d_wo(eqn, values: np.ndarray, x0: float, x1: float, y0: float, y1: float, t0: float, t1: float,
                    savedir="visualization"):
    ...
    for t in range(0, t_bins, tstep):
        ...
        ax.set_ylabel('y', font2)
        ax.set_zlabel('u', font2)
        im = ax.plot_trisurf(x.flatten(), y.flatten(), values[:, :, t].flatten(), triangles=tri.simplices[mask],
                             cmap=plt.cm.coolwarm, alpha=0.7, vmin=zmin, vmax=zmax)
        for ob in eqn.geometry.obstacles:
            pt = ob.sampler(50)
            ax.plot3D(pt[:, 0], pt[:, 1], np.zeros(50, ), color="black")
        cbar = fig.colorbar(im, ax=ax)
...
def stest(vanal_u, models, Nx=1, Nt=1, M=1, Mx=None, Mt=None, Qx=1, Qt=1, w=None, x0=0, x1=1, t0=0, t1=1, test_Qx=None,
...
def test_psib(vanal_u, models, Nx=1, Nt=1, M=1, Mx=None, Mt=None, Qx=1, Qt=1, w=None, x0=0, x1=1, t0=0, t1=1,
              test_Qx=None, test_Qt=None, block=0):
    L = x1 - x0
    tf = t1 - t0
...
def record(main, eqname, recursive_times=1, tf=1.0, time_blocks=1, Nxs=1, Nts=1, Ms=1, Mxs=1, Mts=1, Qxs=1, Qts=1,
           eqn="advect", method="rfm"):
    if method == "rfm":
    ...
    for i, item in enumerate(loop):
        ...
        for k in range(recursive_times):
            ...
            if method == "rfm":
                ...
                print(
                    f"[Iter={k + 1:d}/{recursive_times:d}] Nx={Nx:d}, Nt={Nt:d}, M={M:d}, Qx={Qx:d}, Qt={Qt:d}, time_block={time_block:d}")
                ...
                print(
                    f"[Iter={k + 1:d}/{recursive_times:d}] Nx={Nx:d}, Nt={Nt:d}, Mx={Mx:d}, Mt={Mt:d}, Qx={Qx:d}, Qt={Qt:d}, time_block={time_block:d}")
            ...
            if method == "rfm":
                ...
                L_is, L_2s, e_i, e_2 = main(Nx=Nx, Nt=Nt, Mx=Mx, Mt=Mt, tf=tf, time_block=time_block, Qx=Qx, Qt=Qt,
                                            plot=plot, img_save_dir=img_save_dir)
            elif method == "smul":
                L_is, L_2s, e_i, e_2 = main(Nx=Nx, Nt=Nt, Mx=Mx, Mt=Mt, tf=tf, time_block=time_block, Qx=Qx, Qt=Qt,
                                            plot=plot, img_save_dir=img_save_dir)
        ...
        with open(f"logs/{eqn}_{method}_tf={tf:.2f}.log", fm) as f:
            ...
            if method == "rfm":
                ...
                print(f"Nx={Nx:d}, Nt={Nt:d}, Mx={Mx:d}, Mt={Mt:d}, Qx={Qx:d}, Qt={Qt:d}, t_block={time_block:d}",
                      file=f)
            elif method == "smul":
                print(f"Nx={Nx:d}, Nt={Nt:d}, Mx={Mx:d}, Mt={Mt:d}, Qx={Qx:d}, Qt={Qt:d}, t_block={time_block:d}",
                      file=f)
        ...
        visualize_time_stats(tf=tf, datas=[e_i_res.reshape(recursive_times, -1), e_2_res.reshape(recursive_times, -1)],
                             labels=[r"$L_i$", r"$L_2$"],
                             savename=os.path.join(img_save_dir, f"error_tb={time_block:d}.png"))

        if method == "rfm":
            np.save(
                f"outputs/{eqname}_rfm_tf={tf:.2f}-time_block={time_block:d}_Nx={Nx:d}_Nt={Nt:d}_M={M:d}_Qx={Qx:d}_Qt={Qt:d}/e_i.npy",
                e_i)
            np.save(
                f"outputs/{eqname}_rfm_tf={tf:.2f}-time_block={time_block:d}_Nx={Nx:d}_Nt={Nt:d}_M={M:d}_Qx={Qx:d}_Qt={Qt:d}/e_2.npy",
                e_2)

        elif method == "mul" or method == "smul":
            np.save(
                f"outputs/{eqname}_{method}_tf={tf:.2f}-time_block={time_block:d}_Nx={Nx:d}_Nt={Nt:d}_Mx={Mx:d}_Mt={Mt:d}_Qx={Qx:d}_Qt={Qt:d}/e_i.npy",
                e_i)
            np.save(
                f"outputs/{eqname}_{method}_tf={tf:.2f}-time_block={time_block:d}_Nx={Nx:d}_Nt={Nt:d}_Mx={Mx:d}_Mt={Mt:d}_Qx={Qx:d}_Qt={Qt:d}/e_2.npy",
                e_2)
