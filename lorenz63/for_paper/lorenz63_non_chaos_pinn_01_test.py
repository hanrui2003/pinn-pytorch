...
if "__main__" == __name__:
    ...
    model = torch.load('lorenz63_non_chaos_pinn_01_0.001.pt', map_location=torch.device('cpu'))
    model.eval()
    print("model", model)
    ...
    ax1.plot(t, x, color='b', label='x_hat')
    ax1.plot(t, y, color='g', label='y_hat')
    ax1.plot(t, z, color='k', label='z_hat')
    ax1.plot(t, x_hat, 'r--', label='x')
    ax1.plot(t, y_hat, 'c--', label='y')
    ax1.plot(t, z_hat, color='orange', linestyle='--', label='z')
    ax1.set_xlabel('t', color='black')
    ax1.set_ylabel('u(t)', color='black')
    ax1.legend(loc='upper right')

    ax2.plot(x_hat, y_hat, z_hat, 'r', label='Lorenz 63 model')
    ax2.legend()
    ax2.set_xlabel('x')
    ax2.set_ylabel('y')
    ax2.set_zlabel('z')
