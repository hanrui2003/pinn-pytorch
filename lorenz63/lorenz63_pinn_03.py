...
class LInfiniteLoss(nn.Module):
    ...
    def forward(self, y, y_hat):
        ...
        # max_loss = torch.mean(torch.pow((y - y_hat), 2))
        # max_loss = torch.max(torch.pow((y - y_hat), 2))
        return max_loss
...
class FCN(nn.Module):
    ...
    def __init__(self, layers):
        ...
        self.bns = nn.ModuleList([nn.BatchNorm1d(layers[j]) for j in range(1, len(layers) - 1)])
...
# 初值
x_train_bc = torch.tensor([[0.], [1.]])
y_train_bc = torch.tensor([[-8., 7., 27.], [7.27511711, 6.68299113, 16.00395163]])
...
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, min_lr=1e-6, mode='min', factor=0.5, patience=10000,
                                                       verbose=True)
...
while True:
    ...
    loss_other_pde = PINN.loss_pde(x_train_nf)
...
plt.savefig('./figure/lorenz63_pinn_03_2d.png')
plt.close()
# plt.show()
...
plt.savefig('./figure/lorenz63_pinn_03_3d.png')
# plt.show()
