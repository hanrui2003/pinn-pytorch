...
class FCN(nn.Module):
    ...
    def forward(self, x):
        # 正规化
        a = (x - x_lb) / (x_ub - x_lb)
        for j in range(len(self.linears) - 2):
            z = self.linears[j](a)
            a = self.activation(z)
        # 倒数第二层做Fourier层
        z_fourier = self.linears[-2](a)
        a_fourier = torch.empty_like(z_fourier)
        a_fourier[:, [0]] = z_fourier[:, [0]]
        for k in range(1, z_fourier.shape[1], 2):
            a_fourier[:, [k]] = torch.sin(k * z_fourier[:, [k]])
            a_fourier[:, [k + 1]] = torch.cos((k + 1) * z_fourier[:, [k + 1]])
        return self.linears[-1](a_fourier)
...
if "__main__" == __name__:
    ...
    obs = np.load('lorenz63_rho_28_interval_0.005_total_point_600.npy')
    # 选取观测值
    ...
    plt.savefig('./figure/lorenz63_fnet_obs_01_2d.png')
    plt.close()
    # plt.show()
    ...
    ax.set_zlabel('z')
    ax.set_title('lorenz63')
    plt.savefig('./figure/lorenz63_fnet_obs_01_3d.png')
    # plt.show()
