...
class FCN(nn.Module):
    def __init__(self):
        ...
        self.linear = nn.Linear(in_features=16, out_features=3)
...
if "__main__" == __name__:
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, min_lr=1e-5, mode='min', factor=0.5,
                                                           patience=10000,
                                                           verbose=True)
    epoch = 0
    while True:
    ...
    plt.savefig('./figure/lorenz63_rnn_01_3d.png')
    # plt.show()
