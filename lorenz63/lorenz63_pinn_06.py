...
class FCN(nn.Module):
    ...
    def __init__(self, layers):
        ...
        self.x_dnn = nn.ModuleList([nn.Linear(layers[j], layers[j + 1]) for j in range(len(layers) - 1)])
        self.y_dnn = nn.ModuleList([nn.Linear(layers[j], layers[j + 1]) for j in range(len(layers) - 1)])
        self.z_dnn = nn.ModuleList([nn.Linear(layers[j], layers[j + 1]) for j in range(len(layers) - 1)])

        for linear in self.x_dnn:
            nn.init.xavier_normal_(linear.weight.data)
            nn.init.zeros_(linear.bias.data)

        for linear in self.y_dnn:
            nn.init.xavier_normal_(linear.weight.data)
            nn.init.zeros_(linear.bias.data)

        for linear in self.z_dnn:
            nn.init.xavier_normal_(linear.weight.data)
            nn.init.zeros_(linear.bias.data)

    def forward(self, x):
        # 正规化
        a1 = a2 = a3 = (x - x_lb) / (x_ub - x_lb)
        for j in range(len(self.x_dnn) - 1):
            z1 = self.x_dnn[j](a1)
...
if "__main__" == __name__:
    ...
    plt.savefig('./figure/lorenz63_pinn_06_3d.png')
    # plt.show()
