...
class PIDeepONet(nn.Module):
    ...
    def forward(self, a_trunk, a_branch):
        ...
        # 所以这里用一个变通的方式，批量计算内积
        dot = torch.mul(z_trunk, z_branch)
        return dot

    def loss(self, t_train, initial):
        t_train.requires_grad = True
        y_hat = self.forward(t_train, initial)

        # 计算物理损失
        y_t = autograd.grad(y_hat, t_train, torch.ones_like(y_hat), create_graph=True)[0]
        loss_physics = self.loss_func(y_t, torch.cos(t_train))

        # 计算初值损失
        # 初值点对应index
        ic_idx = [i for i in range(0, len(t_train), n_t)]
        y_hat_ic = y_hat[ic_idx]
        y_label_ic = initial[ic_idx]
...
if "__main__" == __name__:
    ...
    # 生成初值数据，均匀采样
    n_t0 = 200
    initial = torch.rand(n_t0)
    print("initial", initial)

    # 训练点（配置点）
    n_t = 600
    t_train = torch.linspace(0, 2 * torch.pi, n_t)

    initial = initial.unsqueeze(1).tile((1, n_t)).reshape((-1, 1))
    t_train = t_train.tile(n_t0).reshape((-1, 1))

    trunk_layers = [1, 20, 20, 1]
    branch_layers = [1, 20, 20, 1]
    ...
    while True:
        epoch += 1
        loss = PIDON.loss(t_train, initial)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        scheduler.step(loss)
    ...
    torch.save(PIDON, 'sin_don_03.pt')
