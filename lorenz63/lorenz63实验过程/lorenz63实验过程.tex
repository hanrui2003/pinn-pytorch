...
\begin{equation}
	u(t_0,x) \approx \sum_{k=1}^p \sum_{i=1}^n c_i^k \sigma\left(\sum_{j=1}^m W_{bij}^k u(t_j,x_j) + b_{bi}^k \right)
\end{equation}

其中 $u(t_j,x_j)$ 表示在时间 $t_j$ 和空间位置 $x_j$ 上的解，$W_{bij}^k$ 和 $b_{bi}^k$ 是第 $k$ 个分支网络的权重和偏置，$c_i^k$ 是第 $k$ 个分支网络的系数，$\sigma$ 是激活函数。这里没有主干网络的部分，因为初值条件作为参数，不需要主干网络来接收输入。
