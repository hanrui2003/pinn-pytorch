...
二阶导的中心差分：
\begin{equation}
	\frac{d^2u_i}{dx^2} \approx \frac{u_{i-1} - 2u_i + u_{i+1}}{\Delta x^2}
\end{equation}
\begin{equation}
	\frac{d^2\mathbf{u}}{dx^2} \approx \frac{1}{\Delta x^2}
	\begin{bmatrix}
		-2 & 1 & 0 & \cdots & 0 & 0 \\
		1 & -2& 1 & \cdots & 0 & 0 \\
		0 & 1 & -2 & \cdots & 0 & 0 \\
	...
	\begin{bmatrix}
		...
		u_{n} \\
	\end{bmatrix}
	= \frac{1}{\Delta x^2} D^2\cdot \mathbf{u}
\end{equation}
一阶导的中心差分：
\begin{equation}
...
\end{equation}
\begin{equation}
	\frac{d\mathbf{u}}{dx} \approx \frac{1}{2\Delta x}
	\begin{bmatrix}
		0 & 1 & 0 & \cdots & 0 & 0 \\
		-1 & 0 & 1 & \cdots & 0 & 0 \\
		0 & -1 & 0 & \cdots & 0 & 0 \\
	...
	\begin{bmatrix}
		...
		u_{n} \\
	\end{bmatrix}
= \frac{1}{2\Delta x} D^1 \cdot \mathbf{u}
\end{equation}
代码拆解：
\begin{equation}
	\begin{aligned}
		...
		&=\frac{1}{4\Delta x^2} [D^1 \cdot \mathbf{k} \cdot D^1 \cdot \mathbf{u} + 4 \cdot diag(\mathbf{k}) \cdot D^2 \cdot \mathbf{u}] \\
...
\begin{equation}
	\begin{aligned}
		...
		&=\frac{1}{2\Delta x} (\mathbf{v} \cdot D^1 \cdot \mathbf{u} + D^1 \cdot \mathbf{v}\cdot \mathbf{u}) \\
	\end{aligned}
\end{equation}
\begin{equation}
	u_t \approx \frac{u^{n+1}_i - u^n_i}{\Delta t}
\end{equation}
考虑如下方程的差分格式
\begin{equation}
	u_t = (k(x)u_x)_x - (v(x)u)_x
\end{equation}
\begin{equation}
\begin{aligned}
		u^{n+1}_i &= u^n_i + \Delta t \left\{ \frac{k_{i+1} - k_{i-1}}{2\Delta x} \cdot  \frac{u^n_{i+1} - u^n_{i-1}}{2\Delta x} + k_i \frac{u^n_{i-1} - 2u^n_i + u^n_{i+1}}{\Delta x^2} \right . \\ 
		& \left . \qquad - v_i \cdot  \frac{u^n_{i+1} - u^n_{i-1}}{2\Delta x} - \frac{v_{i+1} - v_{i-1}}{2\Delta x} \cdot u^n_i \right\} \\
		 \mathbf{u}^{n+1} &= \mathbf{u}^{n} + \frac{\Delta t}{4(\Delta x)^2} \{ D^1 \cdot \mathbf{k} \cdot D^1 \cdot \mathbf{u}^n + 4 \cdot diag(\mathbf{k}) \cdot D^2 \cdot \mathbf{u}^n \\
		 & \qquad - 2\Delta x \cdot diag(\mathbf{v}) \cdot D^1 \cdot \mathbf{u}^n - 2\Delta x \cdot D^1 \cdot \mathbf{v} \cdot \mathbf{u}^n \} \\
		 &=  \frac{\Delta t}{4(\Delta x)^2} \left\{ \frac{4(\Delta x)^2}{\Delta t} I_n + D^1 \cdot \mathbf{k} \cdot D^1 + 4 \cdot diag(\mathbf{k}) \cdot D^2 \right . \\
		 & \left . \qquad - 2\Delta x \cdot diag(\mathbf{v}) \cdot D^1 - 2\Delta x \cdot D^1 \cdot \mathbf{v} \right\} \cdot \mathbf{u}^n
\end{aligned}
\end{equation}
考虑如下方程的差分格式：（扩散系数D=0.01，反应速率k=0.01；f(x)是源项）
\begin{equation}
	\frac{\partial u}{\partial t}=D\frac{\partial^2 u}{\partial x^2}+ku^2+f(x) \qquad (x,t) \in (0,1] \times(0,1]
\end{equation}
\begin{equation}
	\begin{aligned}
		\frac{u^{n+1}_i - u^n_i}{ \Delta t} &= D \cdot \frac{u^n_{i-1} - 2u^n_i + u^n_{i+1}}{(\Delta x)^2}  + k(u^n_i)^2 + f_i \\ 
		u^{n+1}_i &= u^n_i + \Delta t \cdot \left\{ D \cdot \frac{u^n_{i-1} - 2u^n_i + u^n_{i+1}}{(\Delta x)^2}  + k(u^n_i)^2 + f_i \right\} \\
		\mathbf{u}^{n+1} &= \mathbf{u}^n + \Delta t \left\{ \frac{D}{(\Delta x)^2} \cdot D^2 \cdot \mathbf{u}^n + k(\mathbf{u}^n)^2 + F \right\} \\
	\end{aligned}
\end{equation}
{\color{red} 这里需要注意的是，每次迭代完 $\mathbf{u}^{n+1}$ 的第0个元素和第N个元素是不对的，此时用边值条件重置下。}
\begin{equation}
	1
\end{equation}
...
{\color{red} 代买迁移的坑} \\
要设置精度，jax默认的事32位，会导致精度不够，产生NaN \\
jax对象转成 numpy对象，否则做差分的时候一些便捷操作用不了，$dt = jax.device\_ get(dt)$ \\
CFL条件
\begin{equation}
	\Delta t \leq \frac{\Delta x^2}{2D}
\end{equation}
其中 $\Delta t$ 是时间步长，$\Delta x$ 是空间步长，$D$ 是扩散系数，保证该条件可以保证数值解的稳定性。

\subsection{示例：反应扩散方程-初值问题}
工作站训练数据：
\begin{lstlisting}
# 随机生成的函数个数
stoch_func_num = 5000
...
batch : 54519 lr : 0.0005 loss : 9.648224659031257e-05
Elapsed time:  2:37:43.443680
\end{lstlisting} 
工作站存储的模型文件是：adr\_don\_ic\_01\_gzz\_02.pt，相对误差$10^{-2}$量级。\\
本地训练的结果：网络层数少一点，损失大一点，物理信息取点少一点。在做预测的时候，相对误差也差不太多。\\
精度要想提升，一方面可以继续训练，我觉得更主要的一方面应该在初值函数的选取。目前代码逻辑初值函数是随机正态分布的函数值，然后把两端设置为0，所以靠近两端梯度过大。所以看数值解和神经网络解在初值地方，有差异；\\
...
\begin{align}
	\frac{d\boldsymbol{u}}{dt} &= \boldsymbol{f}(\boldsymbol{u}, t; \boldsymbol{\theta})\
	\boldsymbol{u}(t_0) &= \boldsymbol{u}_0(\boldsymbol{\xi})
\end{align}
其中$\boldsymbol{u}=(u_1, u_2, \dots, u_n)^T$，$\boldsymbol{\theta}$是参数，$\boldsymbol{\xi}$是初始值的参数。我们可以将常微分方程组表示为微分算子$\mathcal{N}$作用在$\boldsymbol{u}$上的形式：
\begin{equation}
	\mathcal{N}(\boldsymbol{u}, t; \boldsymbol{\theta}) = \frac{d\boldsymbol{u}}{dt} - \boldsymbol{f}(\boldsymbol{u}, t; \boldsymbol{\theta})
\end{equation}

然后，我们可以用算子神经网络来逼近微分算子$\mathcal{N}$，形式为：
\begin{equation}
	\mathcal{N}_\theta(\boldsymbol{u}, t) \approx \mathcal{N}(\boldsymbol{u}, t; \boldsymbol{\theta})
\end{equation}

同样地，我们可以用神经网络来逼近初始条件，形式为：
\begin{equation}
	\boldsymbol{u}_\theta(t_0; \boldsymbol{\xi}) \approx \boldsymbol{u}(t_0) = \boldsymbol{u}_0(\boldsymbol{\xi})
\end{equation}

最后，我们可以将整个常微分方程组表示为以下形式：
\begin{align}
	\mathcal{N}\theta(\boldsymbol{u}, t) &= 0\
	\boldsymbol{u}(t_0) - \boldsymbol{u}\theta(t_0; \boldsymbol{\xi}) &= 0
\end{align}
